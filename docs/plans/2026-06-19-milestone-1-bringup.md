...
// TestBackendBoundary enforces ADR-0002: only files under backend/ may reference
// the vendor stack. Run from the repo root.
func TestBackendBoundary(t *testing.T) {
	out, _ := exec.Command("git", "grep", "-l", "-E",
		`gomlx|go-xla|"github.com/gomlx/compute"|pjrt`, "--", "*.go").Output()
